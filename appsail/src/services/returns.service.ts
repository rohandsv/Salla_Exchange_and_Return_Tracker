...
export class ReturnsService {
  ...
  static async listPortalReturns(req: any) {
    const tenantIdRaw = String(req.tenantId || "");
    const portalSession = req.portalSession;

    if (!tenantIdRaw || !portalSession?.order_number) {
      throw new AppError(401, "Unauthorized", "PORTAL_UNAUTHORIZED");
    }
  ...
  /**
   * ✅ Customer cancel a return request (portal)
  ...
  static async cancelPortalReturn(req: any, args: { returnNumber: string; reason?: string }) {
    ...
    if (!tenantIdRaw || !portalSession?.order_number || !portalSession?.contact_hash) {
      throw new AppError(401, "Unauthorized", "PORTAL_UNAUTHORIZED");
    }
    ...
    if (String(rr.customer_contact_hash || "") !== contactHash) {
      throw new AppError(404, "Return not found", "RETURN_NOT_FOUND");
    }

    // Only allow cancel while still requested
    const status = String(rr.status || "").toLowerCase();
    if (status !== "requested") {
      throw new AppError(400, "Return cannot be cancelled at this stage", "RETURN_CANCEL_NOT_ALLOWED", {
        status: rr.status,
      });
    }
    ...
    await ReturnRequestsRepo.update(req, {
      ...
      tenant_id: tenantId,
      status: "cancelled",
      status_reason: args.reason ? String(args.reason) : rr.status_reason ?? null,
      customer_cancelled_at: nowStr,
    });
